...
if [ "$SHELL" = "/usr/bin/fish" ] || [ "$SHELL" = "/bin/fish" ]; then
    ...
    fish -c 'docker run --rm -ti --user foam -v (pwd):/foam/OpenFOAM/user-v2412/work haldardhruv/ubuntu_noble_openfoam:v2412'
# Check if the shell is Zsh
    ...
    docker run --rm -ti --user foam -v $(pwd):/foam/OpenFOAM/user-v2412/work haldardhruv/ubuntu_noble_openfoam:v2412
else
