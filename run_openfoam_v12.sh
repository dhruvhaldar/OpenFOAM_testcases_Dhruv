...
if [ "$SHELL" = "/usr/bin/fish" ] || [ "$SHELL" = "/bin/fish" ]; then
    ...
    fish -c 'docker run -ti -v (pwd):/root/OpenFOAM/12/work haldardhruv/ubuntu_noble_openfoamv12-dhruv'
# Check if the shell is Zsh
    ...
    docker run -ti -v $(pwd):/root/OpenFOAM/12/work haldardhruv/ubuntu_noble_openfoamv12-dhruv
else
